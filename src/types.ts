...
export type AICellKind =
  | "ai.llm"           // generic chat completion → string
  | "ai.embed"         // text → number[] (vector)
  | "ai.image"         // text → image (URL or base64)
  | "ai.translate"     // text → translated text
  | "ai.sentiment"     // text → { label, score }
  | "ai.summarize"     // long text → short text
  | "ai.code"          // description → code (in a language)
...
export type AIResult =
  | string                  // for llm, translate, summarize, code, vision
  | number[]                // for embed
  | { url: string; b64?: string }  // for image
  | { label: string; score: number }  // for sentiment
  ;
...
export interface AIProvider {
  ...
  /** Call the provider. */
  call(config: AIConfig, options: { apiKey: string; signal?: AbortSignal }): Promise<ProviderResponse>;
}
