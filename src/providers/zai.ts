...
const ZAI_MODELS: Record<string, ModelInfo> = {
  "glm-4.5": {
    id: "glm-4.5",
    name: "GLM 4.5",
...
export class ZaiProvider implements AIProvider {
  ...
  private endpoint = "https://api.z.ai/api/paas/v4/chat/completions";
  ...
  async call(
    ...
    if (!apiKey) throw new AIError("zai", 401, "missing api key");
    ...
    switch (config.kind) {
      ...
      default:
        throw new AIError("zai", 400, `unknown cell kind: ${(config as any).kind}`);
    }
  }
  ...
  private async chatCompletion(
    ...
    if ("stream" in config && config.stream) body.stream = true;
  ...
  private async translate(
    ...
    if (config.kind !== "ai.translate") throw new Error("expected translate");
  ...
  private async sentiment(
    ...
    if (config.kind !== "ai.sentiment") throw new Error("expected sentiment");
    ...
    const label = (res.result as string).trim().toUpperCase();
  ...
  private async embed(
    ...
    if (config.kind !== "ai.embed") throw new Error("expected embed");
