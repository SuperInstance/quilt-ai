...
export class DeepseekProvider implements AIProvider {
  ...
  async call(
    ...
    if (!apiKey) throw new AIError("deepseek", 401, "missing api key");
    ...
    switch (config.kind) {
      ...
      case "ai.sentiment":
        ...
        break;
      default:
        throw new AIError("deepseek", 400, `cell kind ${(config as any).kind} not supported on deepseek`);
    }
    ...
    if (!res.ok) {
      ...
      throw new AIError("deepseek", res.status, errText);
    }
