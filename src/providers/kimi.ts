...
export class KimiProvider implements AIProvider {
  ...
  private endpoint = "https://api.moonshot.ai/v1/chat/completions";
  ...
  async call(
    ...
    if (!apiKey) throw new AIError("kimi", 401, "missing api key");
    ...
    switch (config.kind) {
      ...
      case "ai.sentiment":
        ...
        break;
      default:
        throw new AIError("kimi", 400, `cell kind ${(config as any).kind} not supported on kimi`);
    }
    ...
    if (!res.ok) {
      ...
      throw new AIError("kimi", res.status, errText);
    }
    const data = await res.json();
    let result: any = data.choices?.[0]?.message?.content || "";
