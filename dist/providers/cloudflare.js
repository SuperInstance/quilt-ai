...
class CloudflareProvider {
    ...
    async call(config, options) {
        ...
        if (!apiKey)
            throw new types_js_1.AIError("cloudflare", 401, "missing api token");
        if (!this.accountId)
            throw new types_js_1.AIError("cloudflare", 400, "missing account id");
        ...
        switch (config.kind) {
            ...
            case "ai.embed":
                ...
                if (config.input.length > 50000) {
                    throw new types_js_1.AIError("cloudflare", 400, "input too long for embedding model");
                }
            ...
            default:
                throw new types_js_1.AIError("cloudflare", 400, `cell kind ${config.kind} not supported`);
        }
    ...
    async llmCall(url, body, apiKey, signal, config) {
        ...
        if (!res.ok) {
            ...
            throw new types_js_1.AIError("cloudflare", res.status, errText);
        }
