...
class ZaiProvider {
    ...
    async call(config, options) {
        ...
        if (!apiKey)
            throw new types_js_1.AIError("zai", 401, "missing api key");
        // Route by cell kind
        switch (config.kind) {
            case "ai.llm":
            case "ai.code":
            ...
            case "ai.image":
                throw new types_js_1.AIError("zai", 400, "ai.image not supported on z.ai (use cloudflare or openai)");
            default:
                throw new types_js_1.AIError("zai", 400, `unknown cell kind: ${config.kind}`);
        }
    }
    ...
    async chatCompletion(config, apiKey, signal) {
        ...
        if (!res.ok) {
            ...
            throw new types_js_1.AIError("zai", res.status, errText);
        }
